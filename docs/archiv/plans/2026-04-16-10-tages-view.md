...
```bash
TOKEN="<REDACTED — Coolify API Token, siehe app/tasks/todo.md für aktuellen Wert>"
APP_UUID="<REDACTED — App-UUID, gehörte zum inzwischen ersetzten alten Server>"
curl -s -X POST "http://<COOLIFY-HOST>:8000/api/v1/applications/${APP_UUID}/stop" \
  -H "Authorization: Bearer $TOKEN" -H "Accept: application/json"
sleep 3
curl -s -X POST "http://<COOLIFY-HOST>:8000/api/v1/applications/${APP_UUID}/start" \
  -H "Authorization: Bearer $TOKEN" -H "Accept: application/json"
```
...
for i in $(seq 1 20); do
  STATUS=$(curl -s "http://<COOLIFY-HOST>:8000/api/v1/deployments/${DEPLOY_UUID}" \
...
Aktuelle App-URL öffnen (siehe `app/tasks/todo.md`) und Wochenplan-Seite prüfen. (Historischer Hinweis: die im April/August genutzte Adresse `140.82.38.192` ist seit der Server-Migration im September nicht mehr aktuell.)
