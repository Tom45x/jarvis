...
function mockWochenplan(gerichte: Gericht[]): WochenplanGenerierungErgebnis {
  const normale = gerichte.filter(g => g.kategorie !== 'frühstück' && !g.gesperrt)
...
export async function generiereWochenplan(
  ...
  const normaleGerichte = gerichte.filter(g => g.kategorie !== 'frühstück')
...
Erstelle einen Wochenplan für Montag bis Sonntag. Deine Aufgabe: alle 14 Mittag- und Abendessen wählen (Frühstück wird separat befüllt).
